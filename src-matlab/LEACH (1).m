...
% Energy Dissipation for normal nodes %
    ...
    for i=1:n
       if (SN(i).cond==1) && (SN(i).role==0) && (CLheads>0)
        ...
        if SN(i).E<=0       % if nodes energy depletes with transmission (if transmission node energy deplets)
        dead_nodes=dead_nodes +1;
        operating_nodes= operating_nodes - 1
        SN(i).cond=0;
        SN(i).chid=0;
...
% if number of operating nodes becomes less than n , note the number of
% round in which node died as a flag1stdead
